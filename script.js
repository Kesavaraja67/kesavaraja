  ...
  handleCommand(inputElement) {
    ...
    switch (cmd) {
      ...
      case "clear":
        ...
        break;
      case "projects":
        this.printProjectsText(outputElement);
        break;
      case "skills-visual":
        this.showSkills(outputElement);
        break;
...
╚═╝  ╚═╝╚══════╝╚══════╝╚═╝  ╚═╝  ╚═══╝  ╚═╝  ╚═╝</div>`;
    ...
    const welcome =
      ...
      this.wrapWithColor("Full-Stack Developer · Java · React · Open Source\n", "#33FF33") +
      this.wrapWithColor(divider + "\n\n", "#555555") +
  ...
  showAbout(outputElement = this.output) {
    const about = `<span style="color: #33FF33; font-weight: bold;">✨ About Me</span>

${this.wrapWithColor("Passionate Computer Science undergraduate at SRMIST (CGPA 9.03) building production-grade software across the full stack. I've shipped a SaaS PWA with Razorpay payments achieving 95+ Lighthouse scores, engineered microservices with Java Spring Boot & Docker, and contributed +1,896 lines of merged PRs to open-source AI Linux projects. I bring Java + TypeScript depth, open-source credibility, and a relentless drive to build systems that actually matter.", "#33FF33")}`;
...
2. ${this.wrapWithColor("Full-Stack Version (Web & Cloud Focus)", "#E8C96A")}
  ...
  printProjectsText(outputElement = this.output) {
    const projectsText = `<span style="color: #33FF33; font-weight: bold;">📁 PROJECTS</span>

${this.wrapWithColor("1. Neo-Brutalism UI Kit", "#33FF33")}
Stack: React, Tailwind CSS, Framer Motion
• Open-source UI component library featuring high-contrast, bold brutalist aesthetics.

${this.wrapWithColor("2. Terminal Portfolio", "#33FF33")}
Stack: JavaScript, HTML5, CSS3
• Fully functional interactive terminal emulator in the browser with file system and commands.

${this.wrapWithColor("3. Smart Home Dashboard", "#33FF33")}
Stack: Next.js, TypeScript, IoT APIs
• Real-time dashboard for monitoring and controlling smart home appliances.

${this.wrapWithColor("4. Echo Mind Framework — Modular AI Memory", "#33FF33")}
Stack: Python, FastAPI, HTML/CSS/JS
• Designed a modular AI framework simulating memory persistence, contextual reasoning, and adaptive decision-making with pluggable FastAPI components.`;

    const projectsDiv = document.createElement("div");
    projectsDiv.innerHTML = projectsText;
  ...
  showExperience(outputElement = this.output) {
    const experience = `<span style="color: #33FF33; font-weight: bold;">💼 Professional Experience & Projects</span>

<span style="color: #33FF33;">1. 75 Club — Full-Stack SaaS PWA</span>
${this.wrapWithColor("Stack: Next.js 15, TypeScript, FastAPI, PostgreSQL, Supabase, Razorpay, Docker, OCR/AI, Vercel", "#33FF33")}
• Architected and shipped a full-stack SaaS PWA end-to-end — system design, schema normalization, frontend deployment — serving concurrent multi-tenant users at 95+ Lighthouse score.
• Engineered optimized PostgreSQL schemas with indexed queries for high-concurrency multi-tenant access.
• Built RESTful APIs with FastAPI following clean separation of concerns and OOP design principles.
• Integrated Razorpay payment APIs for subscription management and Pro tier upgrades.
• Developed OCR + AI data ingestion pipeline for automated timetable document processing.
• Deployed via CI/CD on Vercel with complete ownership of the full software development lifecycle.

<span style="color: #33FF33;">2. API Rate Limiter — Production-Grade Service</span>
${this.wrapWithColor("Stack: Java 21, Spring Boot, Redis, MySQL, Spring Security, Spring Data JPA, Railway", "#33FF33")}
• Architected a production API rate limiting service in Java Spring Boot implementing sliding window algorithm with Redis sorted sets at O(log n) complexity.
• Engineered a Spring OncePerRequestFilter intercepting every API request for auth and real-time rate limit enforcement.
• Designed 3-layer Spring Boot architecture (Controller → Service → Repository) with constructor-based DI, Spring Data JPA, and @Transactional-safe MySQL persistence.
• Deployed full stack on Railway with cloud MySQL and Redis; built developer dashboard on Cloudflare Pages.

<span style="color: #33FF33;">3. Compliance Sentinel — RESTful Microservice</span>
${this.wrapWithColor("Stack: Python, FastAPI, Docker, SQLite, Pytest", "#33FF33")}
• Developed a production-ready RESTful microservice with 3-layer backend defense architecture — 100% attack block rate, sub-millisecond API response latency.
• Applied OOP design patterns for modular middleware: PII redaction, audit logging, request validation — 86.63% test coverage with 20/20 Pytest tests.
• Containerized the entire stack with Docker for reproducible deployments and rapid horizontal scaling.

<span style="color: #33FF33;">4. NextGen Traffic Insights — Computer Vision</span>
${this.wrapWithColor("Stack: Python, YOLOv8, OpenCV, Priority Queues, Spatial Hash Maps", "#33FF33")}
• Implemented priority queues and spatial hash maps to optimize a real-time vehicle detection pipeline classifying 52+ vehicles/session at stable 30 FPS.
• Fine-tuned YOLOv8 for traffic-specific classification with OpenCV video stream processing; 95% detection accuracy.

<span style="color: #33FF33;">5. Cube Buddy — Algorithmic Computer Vision</span>
${this.wrapWithColor("Stack: Java, Python, OpenCV, BFS, A* Search Algorithm", "#33FF33")}
• Built a real-time Rubik's cube solver in Java using BFS/A* graph traversal with 95% detection accuracy from live camera input.

<span style="color: #33FF33;">6. Echo Mind Framework — Modular AI Memory</span>
${this.wrapWithColor("Stack: Python, FastAPI, HTML/CSS/JS", "#33FF33")}
• Designed a modular AI framework simulating memory persistence, contextual reasoning, and adaptive decision-making with pluggable FastAPI components.`;
  ...
  showEducation(outputElement = this.output) {
    const education = `<span style="color: #33FF33; font-weight: bold;">🎓 Education</span>

${this.wrapWithColor("B.Tech in Computer Science and Engineering (AI & ML)", "#33FF33")}
${this.wrapWithColor("SRMIST, Tiruchirappalli, India | June 2023 – May 2027 | CGPA: 9.03 / 10.0", "#33FF33")}
Relevant Coursework: DSA, OOP, DBMS, OS, Computer Networks, System Design, Software Engineering

${this.wrapWithColor("Higher Secondary (12th Grade — CBSE)", "#33FF33")}
${this.wrapWithColor("Sri Ambal Vidhyalaya CBSE, Vilathikulam, India | 2022–2023", "#33FF33")}`;
  ...
  showSkills(outputElement = this.output) {
    const skills = `<span style="color: #33FF33; font-weight: bold;">🛠️ SKILLS</span>

${this.wrapWithColor("Languages:", "#33FF33")} Java 21 · TypeScript · Python · JavaScript · SQL · HTML/CSS
${this.wrapWithColor("Frontend:", "#33FF33")} React.js · Next.js 15 · Tailwind CSS · PWA · TypeScript (Frontend)
${this.wrapWithColor("Backend:", "#33FF33")} Spring Boot · FastAPI · Node.js · REST APIs · JWT Auth · Microservices
${this.wrapWithColor("Databases:", "#33FF33")} PostgreSQL · MySQL · Redis · MongoDB · SQLite · Supabase
${this.wrapWithColor("DevOps:", "#33FF33")} Docker · Git · GitHub Actions · Vercel · Railway · Linux · SonarQube
${this.wrapWithColor("AI / ML:", "#33FF33")} YOLOv8 · OpenCV · BFS/A* · OCR Pipelines · TensorFlow/CNN · Streamlit
${this.wrapWithColor("Practices:", "#33FF33")} OOP · System Design · Unit Testing · Clean Code · Agile
${this.wrapWithColor("IDE's & Tools:", "#33FF33")} VS Code · Antigravity · Cursor · IntelliJ IDEA · Postman`;
  ...
  showContact(outputElement = this.output) {
    const contact = `<span style="color: #33FF33; font-weight: bold;">📫 Contact Information</span>

${this.wrapWithColor("Email:    krkesavaraja67@gmail.com", "#33FF33")}
${this.wrapWithColor("Website:  kesavaraja.vercel.app", "#33FF33")}
${this.wrapWithColor("GitHub:   github.com/Kesavaraja67", "#33FF33")}
${this.wrapWithColor("LinkedIn: linkedin.com/in/Kesavaraja-m", "#33FF33")}`;
  ...
  generateLinkedInCover(outputElement) {
    ...
    terminalTitle.textContent = "kesavaraja: ~/resume";
...
╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝ ╚═════╝ `;
    ...
    role.textContent = "Full-Stack Developer • Java • React • Open Source";
    ...
    url.textContent = "kesavaraja.vercel.app";
...
// Initialize the terminal if it exists on the page
if (document.querySelector(".terminal-container")) {
  new TerminalResume();
}

// --- NEW SKEUOMORPHIC LOGIC ---

document.addEventListener('DOMContentLoaded', () => {
  // 1. Vault Loader Dismissal
  const loader = document.querySelector('.vault-loader');
  if (loader) {
    // Add open class to trigger swing
    loader.classList.add('vault-open');
    // Hide after 2.5s
    setTimeout(() => {
      loader.classList.add('vault-hidden');
      setTimeout(() => { loader.style.display = 'none'; }, 500);
    }, 2500);
  }

  // 2. Navbar active state and scroll
  ...
  if (mapContainer && typeof L !== 'undefined') {
      ...
      html: `<svg width="24" height="32" viewBox="0 0 24 32">
        <defs>
          <linearGradient id="brass" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <stop offset="50%" style="stop-color:#C9A040"/>
            <stop offset="100%" style="stop-color:#8B6914"/>
          </linearGradient>
        </defs>
        <circle cx="12" cy="10" r="10" fill="url(#brass)" stroke="#5C3317" stroke-width="1.5"/>
        <path d="M12 20 L12 32" stroke="#C9A040" stroke-width="2"/>
    ...
    locations.forEach(loc => {
      L.marker(loc.coords, { icon: brassPin })
        .bindPopup(`<div style="font-family: 'Lora', serif; padding: 5px; background: #EDE0C8; border: 2px solid #C9A040; border-radius: 4px;">${loc.popup}</div>`)
        .addTo(map);
    });
  }
});
